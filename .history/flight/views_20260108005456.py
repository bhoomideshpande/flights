...
try:
    ...
    if len(Flight.objects.all()) == 0:
        print("No flights found in the Database. Adding all flights automatically (non-interactive mode)...")
        addDomesticFlights()
...
def flight(request):
    ...
    if trip_type == '2':
        ...
        # Robust Place lookup for round trip
        try:
            origin2 = Place.objects.get(code=d_place.upper())
        except Place.DoesNotExist:
            try:
                origin2 = Place.objects.get(city__iexact=d_place)
            except Place.DoesNotExist:
                return render(request, "flight/search.html", {"message": f"Origin place '{d_place}' not found."})
        try:
            destination2 = Place.objects.get(code=o_place.upper())
        except Place.DoesNotExist:
            try:
                destination2 = Place.objects.get(city__iexact=o_place)
            except Place.DoesNotExist:
                return render(request, "flight/search.html", {"message": f"Destination place '{o_place}' not found."})
    seat = request.GET.get('SeatClass')

    flightday = Week.objects.get(number=depart_date.weekday())
    # Robust Place lookup for main trip
    try:
        destination = Place.objects.get(code=d_place.upper())
    except Place.DoesNotExist:
        try:
            destination = Place.objects.get(city__iexact=d_place)
        except Place.DoesNotExist:
            return render(request, "flight/search.html", {"message": f"Destination place '{d_place}' not found."})
    try:
        origin = Place.objects.get(code=o_place.upper())
    except Place.DoesNotExist:
        try:
            origin = Place.objects.get(city__iexact=o_place)
        except Place.DoesNotExist:
            return render(request, "flight/search.html", {"message": f"Origin place '{o_place}' not found."})
    if seat == 'economy':
