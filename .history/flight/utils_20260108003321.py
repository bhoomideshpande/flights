...
def cleanDuplicatePlaces():
    print("Cleaning duplicate places...")
    seen = set()
    for place in Place.objects.all():
        key = (place.code, place.city, place.airport, place.country)
        if key in seen:
            place.delete()
        else:
            seen.add(key)
...
def cleanDuplicateWeeks():
    print("Cleaning duplicate weeks...")
    seen = set()
    for week in Week.objects.all():
        key = (week.number, week.name)
        if key in seen:
            week.delete()
        else:
            seen.add(key)
    print("Done cleaning duplicate weeks.\n")
